...
open class GHScanWrapper: NSObject,AVCaptureMetadataOutputObjectsDelegate {
    ...
    // 存储返回结果
    var arrayResult = [GHScanResult]()

    // 扫码结果返回block
    var successBlock: ([GHScanResult]) -> Void
    ...
    init(videoPreView: UIView,
         ...
         success: @escaping (([GHScanResult]) -> Void)) {
    ...
    open func captureOutput(_ captureOutput: AVCaptureOutput,
        ...
        for current in metadataObjects {
            ...
            arrayResult.append(GHScanResult(str: code.stringValue,
                                             img: UIImage(),
    ...
    open func switchCameraTapped(sender: Any) {
        ...
        guard let newCameraPosition = newCamera else {
            return
        }
        ...
        do {
            newVideoInput = try AVCaptureDeviceInput(device: newCameraPosition)
    ...
    /**
     ...
     - returns: 返回识别结果
     */
    public static func recognizeQRImage(image: UIImage) -> [GHScanResult] {
        ...
        return features.filter {
            ...
            GHScanResult(str: $0.messageString,
                          img: image,
    ...
    static func getConcreteCodeImage(srcCodeImage: UIImage, codeResult: GHScanResult) -> UIImage? {
    ...
    // 获取二维码的图像区域
    public static func getConcreteCodeRectFromImage(srcCodeImage: UIImage, codeResult: GHScanResult) -> CGRect {
