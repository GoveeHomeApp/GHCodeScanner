...
open class GHScanView: UIView {
    
    // 扫码区域各种参数
    var viewStyle = GHScanViewStyle()

    // 扫码区域
    var scanRetangleRect = CGRect.zero

    // 线条扫码动画封装
    var scanLineAnimation: GHScanLineAnimation?

    // 网格扫码动画封装
    var scanNetAnimation: GHScanNetAnimation?
    ...
    */
    public init(frame: CGRect, vstyle: GHScanViewStyle) {
        viewStyle = vstyle

        switch viewStyle.anmiationStyle {
        case GHScanViewAnimationStyle.LineMove:
            scanLineAnimation = GHScanLineAnimation.instance()
        case GHScanViewAnimationStyle.NetGrid:
            scanNetAnimation = GHScanNetAnimation.instance()
        case GHScanViewAnimationStyle.LineStill:
...
//        drawScanRect()
    ...
    func drawScanRect() {
        ...
        // 左上角水平线
        context.move(to: CGPoint(x: leftX - linewidthAngle / 2 + 10, y: topY+1))
        context.addLine(to: CGPoint(x: leftX + wAngle + 10, y: topY+1))
        // 左上角弧线
        context.move(to: CGPoint(x: leftX, y: topY - linewidthAngle / 2 + 10))
        context.addArc(center: CGPoint(x: leftX - linewidthAngle / 2 + 10, y: topY + 10), radius: 9, startAngle: -.pi, endAngle: -.pi/2, clockwise: false)
        // 左上角垂直线
        context.move(to: CGPoint(x: leftX-0.5, y: topY - linewidthAngle / 2 + 10))
        context.addLine(to: CGPoint(x: leftX-0.5, y: topY + hAngle + 10))
        
        // 左下角水平线
        context.move(to: CGPoint(x: leftX - linewidthAngle / 2 + 10, y: bottomY - 1))
        context.addLine(to: CGPoint(x: leftX + wAngle + 10, y: bottomY - 1))
        // 左下角弧线
        context.move(to: CGPoint(x: leftX - linewidthAngle / 2 + 10, y: bottomY - 1))
        context.addArc(center: CGPoint(x: leftX - linewidthAngle / 2 + 10, y: bottomY - 10), radius: 9, startAngle: .pi/2, endAngle: .pi, clockwise: false)
        // 左下角垂直线
        context.move(to: CGPoint(x: leftX-0.5, y: bottomY + linewidthAngle / 2 - 10))
        context.addLine(to: CGPoint(x: leftX-0.5, y: bottomY - hAngle - 10))

        // 右上角水平线
        context.move(to: CGPoint(x: rightX + linewidthAngle / 2 - 10, y: topY + 1))
        context.addLine(to: CGPoint(x: rightX - wAngle - 10, y: topY + 1))
        // 右上角弧线
        context.move(to: CGPoint(x: rightX + linewidthAngle / 2 - 10, y: topY + 1))
        context.addArc(center: CGPoint(x: rightX + linewidthAngle / 2 - 10, y: topY + 10), radius: 9, startAngle: -.pi/2, endAngle: 0, clockwise: false)
        // 右上角垂直线
        context.move(to: CGPoint(x: rightX+0.5, y: topY - linewidthAngle / 2 + 10))
        context.addLine(to: CGPoint(x: rightX+0.5, y: topY + hAngle + 10))

        // 右下角水平线
        context.move(to: CGPoint(x: rightX + linewidthAngle / 2 - 10, y: bottomY - 1))
        context.addLine(to: CGPoint(x: rightX - wAngle - 10, y: bottomY - 1))
        // 右下角弧线
        context.move(to: CGPoint(x: rightX + linewidthAngle / 2 - 10, y: bottomY - 1))
        context.addArc(center: CGPoint(x: rightX + linewidthAngle / 2 - 10, y: bottomY - 10), radius: 9, startAngle: .pi/2, endAngle: 0, clockwise: true)
        // 右下角垂直线
        context.move(to: CGPoint(x: rightX+0.5, y: bottomY + linewidthAngle / 2 - 10))
        context.addLine(to: CGPoint(x: rightX+0.5, y: bottomY - hAngle - 10))
        ...
        // 上横线
        context.move(to: CGPoint(x: leftX + wAngle + 10, y: topY+1))
        context.addLine(to: CGPoint(x: rightX - wAngle - 10, y: topY + 1))
        // 右竖线
        context.move(to: CGPoint(x: rightX+0.5, y: topY + hAngle + 10))
        context.addLine(to: CGPoint(x: rightX+0.5, y: bottomY + linewidthAngle / 2 - 10))
        // 下横线
        context.move(to: CGPoint(x: rightX - wAngle - 10, y: bottomY - 1))
        context.addLine(to: CGPoint(x: leftX + wAngle + 10, y: bottomY - 1))
        
        context.move(to: CGPoint(x: leftX-0.5, y: bottomY - hAngle - 10))
        context.addLine(to: CGPoint(x: leftX-0.5, y: topY + hAngle + 10))
    ...
    // 根据矩形区域，获取识别区域
    static func getScanRectWithPreView(preView: UIView, style: GHScanViewStyle) -> CGRect {
