...
#修改spec文件tag
#获取版本号并显示
VersionString=`grep -E "s.version.*=" GHCodeScanner.podspec`
...
LineNumber=`grep -nE 's.version.*=' GHCodeScanner.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" GHCodeScanner.podspec
echo -e "\n${Default}================================================"
